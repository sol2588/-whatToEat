...
export default function SearchTime() {
    ...
    const handleTime = (e: MouseEvent<HTMLButtonElement>) => {
        ...
        actions.updateSearchParams('time', searchParams.time == value ? '' : value);
        ...
        <S_SearchItem>
            ...
            <S_SelectBtnWrapper>
                {timeOption.map((timeData) => (
                    <S_SelectBtn key={timeData.value} value={timeData.value} onClick={handleTime} active={searchParams.time === timeData.value}>
                        {timeData.label}
                    </S_SelectBtn>
                ))}
            </S_SelectBtnWrapper>
